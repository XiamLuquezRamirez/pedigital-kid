                            ...
                            <li class="nav-item">
                                @if ($Grupos->id == $Grup)
                                    <a class="nav-link {!! $active !!}"
                                        href="{{ url('/Contenido/GradosxEstud2/' . $Grad . '/' . $Grupos->grupo) }}">{!! $Grupos->descripcion !!}</a>
                                @else
                                    <a class="nav-link"
                                        href="{{ url('/Contenido/GradosxEstud2/' . $Grad . '/' . $Grupos->grupo) }}">{!! $Grupos->descripcion !!}</a>
                                @endif
                    ...
                    <div class="row">
                        ...
                        @foreach ($Estud as $Est)
                            <div data-aos="zoom-out" class="col-col-md-3 col-lg-3 {{ $Est->grupo }}">
                                <!--widget-area -->
                                <div class="widget-area notepad" style="height: 90%; border-radius: 15px;">
                                    <h5 style="font-size: 14px;" class="sidebar-header">{!! $Est->nombre_alumno . ' ' . $Est->apellido_alumno !!}</h5>
                                    <!-- widget -->
                                    <div class="widget2">
                                        <div class="card">
                                            <div class="card-img">
                                                <!-- image  -->
                                                <a href="#">
                                                    <img class="rounded card-img-top" style="width: 70%"
                                                        src="{{ Session::get('URL') . '/app-assets/images/Img_Estudiantes/' . $Est->foto_alumno }}"
                                                        alt="">
                                                </a>
                                            </div>
                                            <div id="mslogin_{{ $Est->usuario_alumno }}"
                                                style="display: none; padding-top: 10px;" class="alert alert-danger"
                                                role="alert">
                                                Contraseña Incorrecta!
                                            </div>
                                            <div id="div_{{ $Est->usuario_alumno }}"
                                                data-ruta="{{ asset('/Contenido/GradosxAsignatura/' . $Grad) }}">

                                            </div>
                                            <div style="padding: 0" class="card-body">
                                                @if (Session::get('PASW') == 'NO')
                                                    <a href='{{ url('/Contenido/GradosxAsignaturaSinCont/' . $Est->usuario_alumno) }}'
                                                        class="btn btn-secondary btn-block btn-sm">Entrar</a>
                                                @else
                                                    <a id="Btn_HabCon_{{ $Est->usuario_alumno }}"
                                                        onclick="$.HabCont({{ $Est->usuario_alumno }});"
                                                        class="btn btn-secondary btn-block btn-sm">Entrar</a>
                                                @endif
                                            </div>
                                            <!--/card-body -->
            ...
            $.extend({
                ...
                HabCont: function(Usu) {
                    ...
                    if ($("#pasw_" + Usu).length > 0) {
                        ...
                        var Text_Cont =
                            '<br><input type="password"  class="form-control caja" name="pasw"  id="pasw_' +
                            Usu + '" placeholder="Contraseña">';
